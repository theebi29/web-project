...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO elite_members (first_name, last_name, email, phone, plan_type, registration_date) VALUES (?, ?, ?, ?, 'elite', NOW())");
    $stmt->bind_param("ssss", $first_name, $last_name, $email, $phone);
    
    // Execute the statement
    if ($stmt->execute()) {
        // Redirect back to form with success status
        header("Location: elite_sucess.html");
        exit();
    } else {
